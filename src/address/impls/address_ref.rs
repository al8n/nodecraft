...
impl serde::Serialize for HostAddrRef<'_> {
  ...
  where
    S: serde::Serializer,
  {
    use smol_str_0_3::ToSmolStr;
    match &self.kind {
      Repr::Ip(ip) => SocketAddr::new(*ip, self.port)
        .to_smolstr()
        .serialize(serializer),
      Repr::DomainRef(name) => {
        let s = smol_str_0_3::format_smolstr!("{}:{}", name.as_str(), self.port);
...
impl<'a> HostAddrRef<'a> {
  ...
  #[inline]
  pub fn to_owned(&self) -> super::HostAddr {
    match self.kind {
      Repr::Ip(addr) => super::HostAddr::from((addr, self.port)),
      Repr::DomainRef(name) => super::HostAddr::from((name, self.port)),
    }
  }
}
...
mod tests {
  ...
  fn test_basic() {
    ...
    let owned = domain2.to_owned();
    assert_eq!(owned.ip().unwrap(), IpAddr::V4(Ipv4Addr::LOCALHOST));
  }
  ...
  fn test_constructor() {
    ...
    assert_eq!(a.port(), 80);
    let ao = a.to_owned();
    assert_eq!(ao.domain().unwrap(), "www.example.com");
    assert_eq!(ao.port(), 80);
    assert_eq!(ao.fqdn().unwrap(), "www.example.com.");
